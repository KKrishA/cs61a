...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        lambda_vars = signature.rest
        validate_formals(lambda_vars)

        env.define(signature.first, LambdaProcedure(lambda_vars, expressions.rest, env))
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    main = expressions.rest
    return LambdaProcedure(formals, main, env)
    "*** YOUR CODE HERE ***"
    # END PROBLEM 7
...
def make_let_frame(bindings, env):
    ...
    while binding is not nil:
        firstPos = binding.first
        validate_form(firstPos, 2, 2)
        names = Pair(firstPos.first, names)
        vals = Pair(eval_all(firstPos.rest, env), vals)
