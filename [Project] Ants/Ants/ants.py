    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.is_container == True:
                if place.ant.can_contain(self) == True:
                    place.ant.store_ant(self)
            elif self.is_container == True:
                if self.can_contain(place.ant) == True:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        for allBees in self.place.bees[:]:
            allBees.reduce_health(amount) 

        if self.health > amount: #ant is alive, only reduce health by amt
            super().reduce_health(amount)

        elif self.health <= amount: # if ant is dead
            for allBees in self.place.bees[:]:
                allBees.reduce_health(self.damage)
            super().reduce_health(amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew: ## chew tsurn if its currently not chewing
            self.turns_to_chew -= 1
        else:
            if len(self.place.bees):
                self.turns_to_chew = self.chewing_turns
                rando = random_bee(self.place.bees)
                rando.reduce_health(rando.health)
