...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def ifContains(sentence):
        for i in subject:
            for j in split(remove_punctuation(sentence)):
                if lower(i) == lower(j):
                    return True
        return False
    return ifContains
...
def accuracy(typed, source):
    ...
    "*** YOUR CODE HERE ***"
    correct = 0
    smallerList = min( len(typed_words) , len(source_words) )
...
def feline_fixes(typed, source, limit):
    ...
    2
    >>> feline_fixes("pill", "pillage", big_limit) # Don't substitute anything, length difference of 3.3
    3
...
def report_progress(typed, source, user_id, upload):
    ...
    final_progress = correct / len(source)
    upload({'id': user_id, 'progress': final_progress})
...
def fastest_words(match):
    ...
    fastest_words = [[] for i in player_indices]
    for word in range(len(word_indices)):
        fastest_time = time(match, 0, word)
        fastest_player = 0
        for player in range(len(player_indices)):
            if time(match, player, word) < fastest_time:
                fastest_time = time(match, player, word)
                fastest_player = player
        fastest_words[fastest_player].append(get_word(match, word))
